...
### Fixed
- Missing expected output now yields **SKIPPED** (not PASSED) across all four affected SDKs — closed
  the false-pass gap where a run with no golden files reported 100% compliance
- Python CSV loader now auto-detects pipe vs comma delimiter; TPC-H/TPC-DS expected outputs were
  loading as a single column under the previous comma-default
- Java `PassThroughEngine` re-keyed by `Plan` object identity (`IdentityHashMap`) to handle
  duplicate Substrait plan binaries in the TPC-DS suite without hash collisions
- Rust clippy errors in `BenchmarkOp` type alias; C++ GTest FetchContent fallback for CI
  (`libgtest-dev` unavailable on `ubuntu-latest`; fallback downloads and builds from source)

### Added
- **TPC-DS expected outputs**: all 99 reference CSVs generated by DuckDB 1.2.0 at scale-0.01;
  54 queries with rows, 45 empty (correct for scale-0.01 selectivity). Result correctness is now
  fully verifiable for both TPC-H (22/22) and TPC-DS (99/99)
- **Java pass-through integration tests**: 22 TPC-H + 99 TPC-DS queries, keyed by Plan identity
- **Python pass-through integration tests**: 22 TPC-H + 99 TPC-DS queries, keyed by `id(plan_bytes)`
- **Oracle provenance note** added to both TPC-H and TPC-DS READMEs (DuckDB 1.2.0 reference)
- **C++ CI job** added to `sdk-build-test.yml`
- **`datafusion-rust` example** fully implemented — real DataFusion 54.1 + `datafusion-substrait`
  54.1 execution, `substrait` 0.64 protobuf decoding, smoke-test CI job added
- **Lifecycle hooks** (`initialize()`/`cleanup()`) added as default no-ops to Java, Python, and Rust SDKs
- **CI workflows README** at `.github/workflows/README.md` documenting all workflow files
- **Maven Central publishing** configured in `sdk/java/build.gradle`

### Changed
- Interface quick-reference table expanded from 6 to 8 SDKs (C++ and Scala columns added)
- `examples/README.md`: accurate execution-status table distinguishing real from structural examples
- `examples/duckdb-java`: wired real JDBC execution via `SELECT * FROM substrait('<base64>')`
- All stale "stub" / "Option A" / "coming soon" documentation removed
- `COMPLIANCE_SDK_VERSION` in engine-compliance-template.yml bumped to `0.1.1`
- Dead publish jobs removed from `sdk-build-test.yml`; SDK consumption docs updated to "build
  from source" until Maven Central / PyPI publishing is live

### Documentation
- README "SDK Notes" stale-changelog section removed; replaced with concise SDK feature table
- README `.github/workflows/README.md` dead link resolved (file now exists)
- Python troubleshooting comment corrected ("re-installs locally" not "from PyPI")
- Scala SDK key-file list added to README SDK Features section
- C++ README build commands updated to canonical `-S/-B` CMake form
- Go README example `go.mod` require updated to `v0.1.1`
- Rust README `Cargo.toml` dependency fixed from `"1.0"` to `"0.1.1"`
- Scala README `build.sbt` dependency fixed from `"1.0.0"` to `"0.1.1"`
- C++/Scala SDK status table corrected — both have full runners, comparators, loaders,
  and 5 pass-through tests; the "⏳ not yet written" rows were wrong
- Python `__init__.py` `__version__` corrected from scaffold leftover `1.0.0` to `0.1.1`
- All `substrait-compliance-0.1.0-all.jar` references updated to `0.1.1`
- `api/build.gradle` version bumped to `0.1.1`
- TPC-DS `metadata.yaml` changelog note updated to reflect all 99 queries present
